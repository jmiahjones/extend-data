...
# remove the noise variables
h2_tailor <- setdiff(h2_tailor, "noise")
h2 <- setdiff(h2, "noise")
h1 <- setdiff(h1, "noise")
h1_tailor <- setdiff(h1_tailor, "noise")
...
# don't do the first stage of learning again
...
h2_form <- as.formula(paste0("y2c ~ a2c + (a2c):(", paste0(h2_tailor, collapse=" + "), ")^2 - 1"))
...
if(use_mcp){
  ...
  Q2_sel_fit <- cv.glmnet(Q2_big, y2c, penalty.factor = pen_fac)
  col_idx <- "lambda.min"
  coefs_pen <- coef(Q2_sel_fit, s=col_idx)[-1]
  plot(Q2_sel_fit)
} else if(use_lar){
...
Q2_sel_mat <- Q2_big[,M2_hat_Q2,drop=F]
...
if(is.numeric(boots_rob_Q2))
  boots_rob_Q2 <- matrix(boots_rob_Q2, nrow=length(M2_hat_Q2))
print("Bootstrap CIs:")
print("------------------")
...
if(use_mcp){
  ...
  Q1_sel_fit <- cv.glmnet(Q1_mat, y1c, penalty.factor = pen_fac)
  col_idx <- "lambda.min"
  coefs_pen <- coef(Q1_sel_fit, s=col_idx)[-1]
  plot(Q1_sel_fit)
} else if(use_lar){
...
if(is.numeric(boots_rob_Q1))
  boots_rob_Q1 <- matrix(boots_rob_Q1, nrow=length(M1_hat_Q1))
print("Bootstrap CIs:")
print("------------------")
...
save.image("./results/no-noise-run.RData")
